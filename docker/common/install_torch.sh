...
sudo $VAI_ROOT/conda/bin/conda update -n base conda -y

. $VAI_ROOT/conda/etc/profile.d/conda.sh \
    && sudo mkdir -p $VAI_ROOT/conda/pkgs \
    && sudo conda install -y -c conda-forge mamba conda-build \
    && python3 -m pip install --upgrade pip wheel setuptools \
    && sudo conda config --env --remove channels https://repo.anaconda.com/pkgs/main || true \
    && sudo conda config --env --remove channels https://repo.anaconda.com/pkgs/r || true \
    && sudo conda config --env --append channels ${VAI_CONDA_CHANNEL} \
    && sudo conda config --env --append channels conda-forge \
    && export PATH="/home/vitis-ai-user/.local/bin:${PATH}"
...
if [[ ${DOCKER_TYPE} == 'rocm' ]]; then
    ...
    && python3 -m pip install --upgrade pip wheel setuptools \
    && conda config --env --remove channels https://repo.anaconda.com/pkgs/main || true \
    && conda config --env --remove channels https://repo.anaconda.com/pkgs/r || true \
